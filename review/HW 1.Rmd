...
```
*What do you notice?* The first thing that I notice is that followers and average viewers do not seem to be as closely related as I would have expected. In all cases, it seems that each channel has a high number of followers but this does not translate to a high number of views, and views seem to always be much lower than a channel's actual number of followers. \
...
The viewers always seem to be much lower than the followers by a noticeable amount. This is interesting as I would have thought a higher percentage of followers would tune in to streams from the channels they follow. \
...
ggplot(sample_data, mapping = aes(x = followers, y = average_viewers)) + 
  geom_point(alpha = 1) + labs(x = "Number of Followers", y = "Average Viewers", title = "Twitch Channels Average Viewers by Follower Count")
```

The first thing I notice is that the sampled points do not seem to follow any pattern and the scales for the x and y axis make the data very spread out. \

To try to fix this, we will transform the data. \
...
ggplot(sample_data, mapping = aes(x = log_followers, y = log_viewers)) + 
...
```
*What do you see now? How does the relationship look like?* Now, the points look slightly closer and the scales are easier to read. This helps me to see that there could be evidence for a linear relationship. \
...
A 10% increase in followers is associated with a 5.9% increase in the average number of viewers. \
...
pred_data %>% 
  ...
  labs(subtitle = "Fitted Model and Raw Data", 
       ...
       y = "log(viewers)")
```
*Do you think our model describes the relationship well?* This is where I start having trouble, I think the line we fit here does an okay job of representing the data. However, I am not sure if there is a better line of best fit. Especially, where the follower counts are greater, as the line seems to not reflect the pattern as well when we move past 6. Additionally, the points before 4 on the log(followers) seem to not be reflected well by our line.\
...
pred_data %>% 
  ...
  labs(title = "Resideuals and Followers", 
       ...
       y = "Squared Residuals")
```
*What do you see? Are there any big residuals?* I see that most of the points are centered around 0, which is a good sign I believe. However, there are some points that have bigger residuals. I squared the values so that I am only working with positive values. As the log(followers) approach 6, there seems to be high values for the residuals. Additionally, right after 5 for the log(followers) there is a small spike in the values of the squared residuals.\

*Do they happen often in a particular range of our x-variable?*\

As log(followers) approaches 6, I see the biggest deviations from 0 for the residuals. There are four points (two on the negative and two on the positive side of 0) that show this high deviation. \
...
*Interpret the results.* Since there are a lot of coefficients here I will try to interpret a few of them. 
I think that English, the reference category is the intercept meaning that there is on average 5112 viewers for English channels. So, looking at Arabic for example, the average viewership for these channels is 5112.81 + 569.39 = 5682.2 average viewers. Let's look at Greek-speaking channels. The average viewership for these channels is 5112.81 - 3151.81 = 1961 average viewers. If we look back at our tibble showing the raw data numbers, these numbers make sense. 
...
pred_data2 %>% 
  ...
  labs(subtitle = "Fitted Model and Raw Data", 
       title = "Followers & Average Viewership", 
       x = "log(followers)", 
       y = "log(viewers)") + theme(axis.text.x = element_text(angle = 90))
```

*How is my prediction doing?* Based on these few I calculated, the prediction seem to be spot on. 
