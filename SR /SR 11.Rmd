...
Using the logit link with Poisson is an interesting idea because logit is used when the outcome is all 0s and 1, and we use Poisson as the likelihood when thte outcome is an all positive count for something. However, we can also have a y=0 with Poisson, so I feel like there MIGHT be a way to use both together. If we are trying to count something where the only possible outcome for the count is 0 or 1, then we could potentially use these two things together. It is hard for me to conceptualize a case when this would be true.

[NEED TO CONTINUE THIS, thinking something like counting people who lose hearing in one ear.... idk) \
